...
namespace AkkaGrpcClient
{
    public class StateManager : IStateManager
    {
        ...
        public Task SaveStateAsync(CancellationToken cancellationToken = default)
        {
            ...
            {
                Console.WriteLine(item.Key+"--"+item.Value.Type+"--"+JsonSerializer.Serialize(item.Value.Value, _serializerOptions) +"---"+item.Value.ChangeAction);
        ...
        public  Task SetStateAsync<T>(string stateName, T value, CancellationToken cancellationToken = default)
        {
            ...
            {
                this.stateChangeTracker.TryAdd(stateName, StateMetadata.Create(value, StateChangeAction.Add));
            }
            else
            {
